...
def lambda_handler(event, context):
    ...
    elif message.get('Event', None) not in VALID_EVENTS:
        logger.info('Event %s is not configured for this Lambda function', message['Event'])
        return
    else:
        # Define the message color
        if message['Event'] == EVENT_INSTANCE_LAUNCH:
            color = 'good'
        elif message['Event'] == EVENT_INSTANCE_TERMINATE:
            color = 'warning'
        elif message['Event'] == EVENT_LAUNCH_ERROR or message['Event'] == EVENT_TERMINATE_ERROR:
            color = 'danger'
        
        # Create the message
        msg = json.dumps(build_autoscaling_message(sns['Subject'], message, color))
